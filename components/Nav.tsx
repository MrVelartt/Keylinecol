    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled
            ? 'bg-[#07100700]/80 backdrop-blur-2xl border-b border-white/[0.05]'
            : 'bg-transparent'
        }`}
        style={scrolled ? { backgroundColor: 'rgba(7,16,7,0.82)' } : {}}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-12 h-[72px] flex items-center justify-between gap-8">
          ...
          <a href="#" className="flex items-center gap-3 group shrink-0">
            ...
            <div className="flex flex-col leading-none">
              <span className="font-display text-[1.1rem] font-semibold tracking-tight">
                KEY<span className="text-[#8DB76A]">LINE</span>
              </span>
              <span className="text-[9px] text-white/30 tracking-[0.25em] uppercase font-light mt-0.5">
                Colombia
              </span>
            </div>
          </a>
          ...
          <div className="hidden lg:flex items-center gap-1.5 font-mono text-[10px] text-white/20 tracking-widest select-none">
            <span className="w-1 h-1 rounded-full bg-[#8DB76A] animate-pulse" />
              ...
              return (
                <a
                  ...
                  className={`relative px-4 py-2 text-[13px] tracking-wide transition-colors duration-200 rounded-lg ${
                    isActive ? 'text-[#8DB76A]' : 'text-white/45 hover:text-white/80'
                  }`}
                >
                  {isActive && (
                    <motion.span
                      layoutId="nav-pill"
                      className="absolute inset-0 bg-[#8DB76A]/[0.08] border border-[#8DB76A]/20 rounded-lg"
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-[#8DB76A] text-[#0a120a] rounded-full text-[13px] font-semibold hover:bg-[#9ec87a] transition-all duration-200 hover:scale-105 hover:shadow-[0_0_20px_rgba(141,183,106,0.35)]"
            >
          ...
          >
            <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col gap-1">
              ...
              <a
                ...
                rel="noopener noreferrer"
                onClick={() => setMenuOpen(false)}
                className="mt-3 flex items-center justify-center gap-2 py-3.5 bg-[#8DB76A] text-[#0a120a] rounded-full font-semibold text-sm"
              >
