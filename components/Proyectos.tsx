...
function TerrainSVG({ project }: { project: Project }) {
  return (
    <svg viewBox="0 0 310 165" className="w-full h-full" preserveAspectRatio="xMidYMid slice">
      ...
      {/* Data overlays */}
      <text x="8" y="14" fill="rgba(149,193,31,0.45)" fontSize="7.5" fontFamily="monospace" letterSpacing="0.08em">
        {project.elv}
      </text>
      <text x="8" y="24" fill="rgba(149,193,31,0.25)" fontSize="6.5" fontFamily="monospace" letterSpacing="0.06em">
        {project.id}
      </text>
    </svg>
  )
...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex flex-col border border-white/[0.07] rounded-2xl overflow-hidden bg-[#2C2C2A] hover:border-[#95C11F]/25 transition-all duration-500 hover:bg-[#333331]"
    >
      {/* Terrain visual */}
